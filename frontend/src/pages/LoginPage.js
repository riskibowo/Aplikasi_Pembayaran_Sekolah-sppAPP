...
const LoginPage = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/school-profile`).then(res => setSchoolProfile(res.data)).catch(() => { });
  }, []);
      ...
      <Card data-testid="login-card" className="w-full max-w-md relative z-10 shadow-2xl border-0 backdrop-blur-sm bg-white/90">
        <CardHeader className="text-center space-y-4 pb-8">
          ...
          <div>
            ...
            <CardDescription className="text-base mt-2">{schoolProfile?.nama_sekolah || "SMK MEKAR MURNI"}</CardDescription>
          </div>
        </CardHeader>
